...
Command.compilePosition = function (position) {
  switch (position.type) {
    ...
    case 'mouse':
      return () => {
        Scene.sharedPoint.x = Input.mouse.sceneX
        Scene.sharedPoint.y = Input.mouse.sceneY
        return Scene.sharedPoint
      }
  }
}
...
Command.transition = function ({variable, start, end, easingId, duration, commands}) {
  ...
  const checkLoopCond = () => {
    if (Event.transitions[0].elapsed < duration) {
      CommandIndex = 0
      return Event.wait(0)
    } else {
  ...
  switch (operation) {
    case 'set': {
      ...
      return () => {
        const element = getElement()
        if (element) {
          UI.setPointerEventRoot(element)
        }
        return true
      }
    }
