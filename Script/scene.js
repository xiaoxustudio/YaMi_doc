...
class SceneContext {
  ...
  async load() {
    ...
    this.ambient = this.savedData?.ambient ?? scene.ambient
      ...
      case 'square': {
        ...
        for (let i = 0; i < length; i++) {
          for (const actor of cells[i]) {
            ...
            if (distX <= half && distY <= half && inspector(actor.teamId, teamId)) {
      ...
      case 'circle': {
        ...
        for (let i = 0; i < length; i++) {
          for (const actor of cells[i]) {
            ...
            if (dist <= radius ** 2 && inspector(actor.teamId, teamId)) {
              CacheList[count++] = actor
            }
          }
        }
      ...
      case 'rectangle': {
        ...
        for (let i = 0; i < length; i++) {
          for (const actor of cells[i]) {
            ...
            const distX = Math.abs(x - actor.x)
            const distY = Math.abs(y - actor.y)
            if (distX <= halfW && distY <= halfH && inspector(actor.teamId, teamId)) {
              actors.push(actor)
            }
          }
        }
  ...
  static actorInspectors = new class {
    // 检查器 - 判断敌对角色
    'enemy' = (teamId1, teamId2) => {
      return Team.isEnemy(teamId1, teamId2)
    }

    // 检查器 - 判断友好角色
    'friend' = (teamId1, teamId2) => {
      return Team.isFriendly(teamId1, teamId2)
    }

    // 检查器 - 判断小队角色
    'team' = (teamId1, teamId2) => {
...
class SceneRegion {
  ...
  getRandomPosition(terrain = -1) {
    ...
    let y = 0
    let count = 0
    const {terrains} = this.parent.scene
    do {
